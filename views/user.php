<html>
<head>
	<title>Problems to solve | <?php echo hsc($student['name']); ?></title>
...
<body>
	<?php
	render('../templates/header_student',array('student_name' => hsc($student['name'])));
	?>
	<div class="body1">
		<div id="notification" onclick="remove_noti()">
			<span id="noti_text"> Hey this is a notification.</span>
	...
	for(j=0;j<=1;j++)
	{
		...
		else
		{
			for(i=0;i<data[j].length;i++)
			{
				...
				tr.innerHTML ='<td><a href="" target=_blank></a></td><td><a title="Submit Solution on SPOJ" href="" target=_blank></a></td><td class="text-center"><a title="Submit/Update final solution" href="" target="_blank"><img src="" height=20px/></a></td><td class="text-center"><a title="View Solution" href="" target="_blank"><img src="" height=20px/></a></td>';
				var prob = data[j][i];
				...
				links[1].setAttribute("href", "<?php echo CC ?>status/" + prob[0] + "?handle=<?php echo hsc($student['handle']);?>");
				links[2].setAttribute("href", "/submit/" + prob[0]);
				links[3].setAttribute("href", "/viewsolution/" + prob[0]);
				...
				table_cell = table_cell.nextSibling;
				table_cell.getElementsByTagName('IMG')[0].setAttribute("src","html/images/plane-"+ section_color(j) +".png");
				table_cell = table_cell.nextSibling;
...
function get_content()
{
	...
	$.ajax({
		url: "/codechef",
		data: { user : '<?php echo hsc($student['handle']);  ?>'},
		type: "GET",
